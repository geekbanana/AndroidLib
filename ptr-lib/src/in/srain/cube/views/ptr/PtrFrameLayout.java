...
public class PtrFrameLayout extends ViewGroup {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderView != null) {
            ...
            //TODO 修改,
            /* 为了header,footer拉出的部分颜色一致, 给header设置了很大的paddingTop,
               给footer设置了很大的paddingBottom, 但为了能正常触发refresh/loadmore,
               这里给mPtrIndicator设置高度时需重新计算.
            */
//            mPtrIndicator.setHeaderHeight(mHeaderHeight);
            mPtrIndicator.setHeaderHeight(mHeaderHeight- Math.abs(mHeaderView.getPaddingTop() - mHeaderView.getPaddingBottom()));
        }
...
//            mPtrIndicator.setFooterHeight(mFooterHeight);
            mPtrIndicator.setFooterHeight(mFooterHeight - Math.abs(mFooterView.getPaddingTop() - mFooterView.getPaddingBottom()));
        }
...
//        }
    ...
    private byte mType = TYPE_NONE;//当前所处的类型
    private final static byte TYPE_NONE = 1;//没有加载任何数据
    private final static byte TYPE_REFRESHING = 2;//正在下拉刷新
    private final static byte TYPE_LOADING_MORE = 3;//正在上拉加载

    /**
     *  如果处在下拉刷新状态,则返回true
     * @return
     */
    public boolean isRefreshingEnhance(){
        return mType == TYPE_REFRESHING;
    }
     ...
     * @return
     */
    public boolean isLoadingMoreEnhance(){
        return mType == TYPE_LOADING_MORE;
    }

    /**
     * 如果是下拉刷新或者上拉加载状态,都会返回true
     * @return
     */
    ...
    public void setContentView(View content){
        if (mContent != null && content != null && mHeaderView != content) {
            removeView(mContent);
        }
